...
it for style adaptation (same call Cozy made for STT).
Usage: train_sft.py [--steps N] [--tag NAME] [--smoke]
"""
import argparse
import json
import os
...
def main() -> None:
    ...
    if a.smoke:
        train_rows = (grammar[:150] + speech * 5)[:200]
    print(f"sft rows: {len(train_rows)} (grammar={len(grammar)} speech={len(speech)})")
    ...
    args = SFTConfig(
        output_dir=os.path.join(OUT, f"lora-{a.tag}"),
