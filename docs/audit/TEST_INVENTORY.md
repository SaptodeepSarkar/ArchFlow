...
`cargo test --workspace` passed 66 tests on 2026-09-16, with 1 hardware streaming test ignored. The current breakdown is 36 core unit tests, 4 core config integration tests, 6 protocol-abuse tests, 4 portable desktop tests, 3 worker pipe tests, and 14 daemon tests (including package-validation coverage). The run emitted warnings, but no test failures.
